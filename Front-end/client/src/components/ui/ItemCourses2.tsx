...
export default function ItemCourses2(props: IProp) {
  const router = useRouter();
  const { courses } = props;
  return (
    <div className="group flex max-w-full rounded-xl overflow-hidden bg-white p-4 border-gray-300 border-2 shadow-[0_0_0_rgb(224,224,224)] duration-700 hover:shadow-[9px_9px_0px_rgb(224,224,224)] hover:duration-700 text-black ease-out hover:translate-y-0 transition-all">
      {/* Image Section */}
      <div className="relative w-1/3 rounded-[10px] overflow-hidden">
        <CldImage
          className="h-[250px] rounded-[10px]"
          src={courses?.image ?? ""}
          alt="d"
          width={350}
          height={250}
